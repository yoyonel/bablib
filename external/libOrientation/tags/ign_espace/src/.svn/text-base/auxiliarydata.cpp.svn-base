...
namespace OrientationMATIS
{
	...
	Date::Date()
	{
		m_seconde = m_annee = m_mois = m_jour = m_heure = m_minute = 0;
	}
	...
	void Date::Set(short annee, unsigned char mois, unsigned char jour, unsigned char heure, unsigned char minute, float seconde, const std::string &time_system)
	{
		...
		AuxiliaryData::AuxiliaryData() :
			m_camera_name(""), m_image_date(Date()), m_calibration_date(Date()), m_exposure_time(0.)
		{
		}

		const std::vector<Sample> & AuxiliaryData::GetSamples() const { return m_samples; }
