...
int main(int argc, char **argv) {
	if (argc < 2) {
		std::cout<<"Can not find test file name in command line! "<<std::endl;
		return -1;
	}
	...
	shared_ptr<ModeleProjection> ori =
			ModeleProjection::ReadFile(oriname);

	TiXmlDocument doc(nomspl.c_str());
	if (!doc.LoadFile()) 
    {
		throw std::logic_error("Can not find *.spl file!");
		//			return shared_ptr<ModeleProjection>();
	}
	TiXmlHandle h_Doc(&doc);
	std::vector <OrientationMATIS::Sample> mysamples;
	OrientationMATIS::XmlHandler::ReadSamples(&h_Doc, mysamples);
//	std::cout<<"lecture de mysamples fini ! \n";
	ori->GetAuxiliaryData().AddSamples(mysamples);
//	std::cout<<"mysamples added to ori ! \n";

	if (!ori->InternalCheck(0.000001))
	{
		std::cerr << "InternalCheck failed for " << oriname << " and " << nomspl << std::endl;
		return -1;
	}
//	std::cout<<" internal check fini ! \n";
	ModeleProjection::WriteFile(ori, orixmlname);

	shared_ptr<ModeleProjection> ori2=
			ModeleProjection::ReadFile(orixmlname);
//	std::cout<< "number of samples after reading : "<<ori2->GetAuxiliaryData().GetSamples().size()<<std::endl;
	...
	{
		std::cerr << "Nb Samples changed from " << nomspl << " to " << orixmlname << std::endl;
		return -1;
	}
	if (!ori2->InternalCheck(0.000001))
	{
