...
class Proj3d {
    public:
        Proj3d(double d = 1);       // matrice diagonale avec la valeur 'd' sur la diagonale (l'identitÃÂ© par dÃÂ©faut)
        ...
        Proj3d(double xmin, double xmax, double ymin, double ymax, double zmin, double zmax); // [xmin,xmax]*[ymin,ymax]*[zmin,zmax] --> [0,1]ÃÂ³
    ...
    //---------------------------
        ...
        static Proj3d rotation(double theta, Vec3d n, bool isNormed = false);    // prÃÂ©condition : isNormed ou ||n||=1
        ...
        static Proj3d U2C();   // unit [0,1]ÃÂ³ --> centered [-1,1]ÃÂ³
        static Proj3d C2U();   // centered [-1,1]ÃÂ³ --> unit [0,1]ÃÂ³
        
    // opÃÂ©rations matricielles :
    //--------------------------
        Proj3d operator*(Proj3d P) const;   // composition
        Vec4d operator*(Vec4d v) const;     // application ÃÂ  un vecteur 4
        Vec3d operator*(Vec3d v) const;     // application ÃÂ  un vecteur 3
    ...
    //-------------------------
        inline const double* coefs() const { return M; }  // le tableau en mÃÂ©moire contentant les 16 coefficients, rangÃÂ©s par colonnes
        inline       double* coefs()       { return M; }  // le tableau en mÃÂ©moire contentant les 16 coefficients, rangÃÂ©s par colonnes
    ...
    //----------------------------------------
        ...
        static Proj3d getTransformGL();        // composÃÂ©e des deux prÃÂ©cÃÂ©dentes
