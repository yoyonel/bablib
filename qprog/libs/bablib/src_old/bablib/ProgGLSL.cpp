...
namespace {
    QString toQS(ShaderGLSL::Type type) {
        return (type == ShaderGLSL::VERTEX) ? "vertex" : "fragment";
        }
    
    ProgGLSL::Locator createDefaultLocator() {
        ProgGLSL::Locator locator(".vert", ".frag");
...
int ProgGLSL::getAttribID(QString name) const {
    if (!isLinked()) {
        Message::error("le programme doit ÃÂªtre linkÃÂ© avant de pouvoir accÃÂ©der ÃÂ  ses attributs");
        return -1;
        }
...
void ProgGLSL::loadDir(QString dirName, bool sub)                   { load(locator.sourcesDir(dirName, sub)); }
void ProgGLSL::load(QString name)                                   { load(locator.sources(name)); }
...
QString ProgGLSL::Source::preprocess(QString code) const {
    ...
    return modif ? preprocess(newCode) : newCode;   // to handle recursive includes
    }
...
ProgGLSL::Locator::Locator(QString vertExt, QString fragExt) :
    vertExt(vertExt), fragExt(fragExt) {
    }
...
ProgGLSL::SourceSet ProgGLSL::Locator::sources(QString fileNamePrefix) const {
    SourceSet res;
...
ProgGLSL::SourceSet ProgGLSL::Locator::sourcesDir(QString dirName, bool sub) const {
    if (sub) {
    ...
    return sourcesDir(ShaderGLSL::VERTEX, dirName) + sourcesDir(ShaderGLSL::FRAGMENT, dirName);
    }
...
QString ProgGLSL::Locator::suffix(ShaderGLSL::Type type)                        const { return type == ShaderGLSL::VERTEX ? vertExt : fragExt; }
