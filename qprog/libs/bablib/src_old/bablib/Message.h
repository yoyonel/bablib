...
class Message {
    ...
    private:
        static QTextStream *errorStream;
        static QTextStream *warningStream;
...
#define MSG_POINT (QString("%1, line %2").arg(__FILE__).arg(__LINE__))
...
struct MsgInfoProcess {
    MsgInfoProcess(QString msg, int sub=0) : prefix(4*sub, ' ') { MSG_INFO(prefix + "- %1 ...", (msg)); }
    void done() { MSG_INFO(prefix + "> done : %1 s.", (chrono.elapsedSecs())); }
    Chrono chrono;
    QString prefix;
    };
