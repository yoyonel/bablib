...
class Kernel2D {
    public:
        //Kernel2D(int R = -1, int* tab = NULL);   // précondition : <R> >= 0 et {<tab> == NULL ou <tab> est de taille (2R+1)*(2R+1)}
        Kernel2D(int R = -1);   // précondition : <R> >= 0
        void destroy();
        
        int radius() const { return R; }
        float& operator()(int i, int j) { return tab[N*(i+R) + (j+R)]; }    // précondition : tab != NULL
        ...
        static Kernel2D constantKernel(int R);  // valeur choisi pour que le noyau soit normalisé
