...
Vec2  Vec2::toPolar() const { return Vec2(norm(), arg()); }
...
Vec2 Vec2::polar(float radius, float theta) { return Vec2(radius * cosf(theta), radius * sinf(theta)); }
Vec2 Vec2::polar(const Vec2 &p) { return polar(p.x, p.y); }
