...
FrameBuffer FrameBuffer::create_Color_Z(int w, int h, GLenum colorBufferFormat, GLenum depthBufferFormat) {
    FrameBuffer fb(w, h);
    fb.create();
    fb.attachRenderBuffer(GL_COLOR_ATTACHMENT0_EXT, colorBufferFormat);
    fb.attachRenderBuffer(GL_DEPTH_ATTACHMENT_EXT,  depthBufferFormat);
    fb.checkCompleteness(false);
    return fb;
    }
        
FrameBuffer FrameBuffer::create_Color(int w, int h, GLenum colorBufferFormat) {
    FrameBuffer fb(w, h);
    fb.create();
    fb.attachRenderBuffer(GL_COLOR_ATTACHMENT0_EXT, colorBufferFormat);
    fb.checkCompleteness(false);
