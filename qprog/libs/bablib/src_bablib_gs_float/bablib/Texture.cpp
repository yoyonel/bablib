...
Texture Texture::createTex2DRect(int w, int h, GLint internalFormat, GLenum interpMode, GLenum wrapMode) {
    Texture tex(GL_TEXTURE_RECTANGLE_ARB, interpMode, wrapMode);
    tex.init();
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, internalFormat, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);   // arguments 7 et 8 quelconques
    tex.loaded = true;
    return tex;
    }

Texture Texture::createTex2DShadow(int w, int h, GLint internalFormat, GLenum interpMode, GLenum wrapMode) {
    Texture tex(GL_TEXTURE_2D, interpMode, wrapMode);
    tex.init();
    glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, w, h, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_SHORT, NULL);   // arguments 7 et 8 quelconques
    tex.loaded = true;
    return tex;
    }
...
void Texture::initAutoMipMapGeneration( GLenum _min_filter, GLenum _mag_filter, GLint _max_levels, GLuint _base_level, GLenum _hint_mode ) {
	...
	GLint n_mipmaps = _max_levels;
	if ( _max_levels == -1 ) {
		n_mipmaps = (GLint)( fmax( log2f(getWidth()), log2f(getHeight()) ) );
		Message::info( QString("# Nb MipMap to generate: ") + QString::number( n_mipmaps ) );
	}
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, n_mipmaps);
	//
