        ...
        public:
            ...
            ChronoGPU(double period) : period(static_cast<GLuint64EXT>(period * 1e6)), n(0) {
                if (period > TimerGPU::maxPeriod())
                    MSG_ERROR("prÃÂ©cision des timer queries (%1 bits) insuffisante pour la pÃÂ©riode spÃÂ©cifiÃÂ©e", (TimerGPU::precision()));
                }
    ...
    class ChronoGPU {
        public:
            ...
            ChronoGPU(float period) : period(period), n(0) {
                if (period > TimerGPU::maxPeriod())
                    MSG_WARNING("prÃÂ©cision des timer queries (%1 bits) potientiellement insuffisante pour la pÃÂ©riode spÃÂ©cifiÃÂ©e", (TimerGPU::precision()));
