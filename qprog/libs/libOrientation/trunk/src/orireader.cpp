...
namespace OrientationMATIS
{
    shared_ptr<ModeleProjection> axono_ori_reader(const std::string &filename)
    {
        ...
        AuxiliaryData data;
        Transfo2D transfo2D;
        return shared_ptr<ModeleProjection> (new ModeleProjectionAxono(nc, nl, data, transfo2D, systemeGeodesie, extr, cPas, lPas, cPPA, lPPA));
    }
    ...
    {
        bool verbose = false; 
        if (verbose) std::cout << "fonction  conic_ori_reader" << std::endl;
        std::ifstream fic(filename.c_str());
        fic.precision(12);
        if (!fic.good())
            return shared_ptr<ModeleProjection> ();
        if (verbose) std::cout << "Fichier est bon" << std::endl;
        std::string typFic;
        fic >> typFic;
        if (verbose) 	std::cout << "typfic =" << typFic <<")"<< std::endl;
        if (::strcmp(typFic.c_str(), "TEXT") != 0)
            return shared_ptr<ModeleProjection> ();
        if (verbose) 	std::cout << "Fichier TEXT" << std::endl;
        int distor;
        fic >> distor;
        ...
        aux.SetSensorSize(imNs, imNl);
        aux.SetAltiSol(altiSol);
        aux.SetPixelSize(pixSize[0],pixSize[1]);
        ...
        {
            nomSysteme.precision(10);
            nomSysteme << Geodesie::NomSysteme(zoneLambert);
        }
        ...
        else if (distor == 2)
        {
            ...
            fic >> cr3 >> cr5 >> cr7;
            fic >> xPPS >> yPPS;
            DistortionPolynomeIGN * polyIGN = new DistortionPolynomeIGN(xPPS, yPPS, cr3, cr5, cr7, true);
            //Fixer la limite de validite du polynome de distorsion
            polyIGN->SetR2max(imNs, imNl);
            shared_ptr<Transfo2DElementaire> tr = shared_ptr<Transfo2DElementaire> (polyIGN);
            t2D.AddTransformation2D(tr);
        }
        if (verbose) 	std::cout << " fin de conic_ori_reader!" << std::endl;
        ...
        shared_ptr<Euclidien> euclidien(new EuclidienMATISRTL(localOrigin[0], localOrigin[1]));

        ModeleProjectionConique * p_mpc = new ModeleProjectionConique(
                imNs, imNl, aux, t2D, nomSysteme.str(), euclidien, extr, focal, PPxy[0], PPxy[1]);
        p_mpc->SetPixelSize(sqrt(pixSize[0]*pixSize[1])); // BV: best we can do, better than nothing

        return shared_ptr<ModeleProjection> (p_mpc);
    }
}
