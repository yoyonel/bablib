...
namespace OrientationMATIS
{
	...
	void ModeleProjectionPhysique::ImageToLocalVec(double c, double l, double &x0, double &y0, double &z0, double &x1, double &y1, double &z1) const
	{
        ...
        bool ModeleProjectionPhysique::IsInfrontOfImage(double x, double y, double z)
        {
            double c,l;
            double x0,y0,z0,x1,y1,z1;
            this->LocalToImage(x,y,z,c,l);
            this->ImageToLocalVec(c,l,x0,y0,z0,x1,y1,z1);
            double cosval = (x1-x0) * (x-x0) + (y1-y0) * (y-y0) + (z1-z0) * (z-z0);
