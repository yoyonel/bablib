...
EnvRenderer::EnvRenderer(QString shaderName, bool writeZ) : writeZ(writeZ) {
    /*
    if (lib)
        prog.load(ProgGLSL::globalDir.sourcesDir(shaderName));
    else
        prog.loadDir(shaderName);
    */
    //prog.load((lib ? ProgGLSL::globalDir : ProgGLSL::dir).sourcesDir(shaderName));
    prog.loadDir(shaderName);
    }
...
//EnvMapRenderer::EnvMapRenderer(Texture *texCubeMap, float *brightness) : EnvRenderer("envmap", true, false) {
EnvMapRenderer::EnvMapRenderer(Texture *texCubeMap, float *brightness) : EnvRenderer("[bablib]/envmap", false) {
    //prog.loadDir("back_envmap");
